...
set.seed(62)
  ...
  Dppb <- seq(0,150,5)  # defines simulated increase in concentration (start, end, steps)
...
plot(Dppb, sim.mean$SP_source , ylim = c(-800,00),ylab=expression(delta~" [\u2030]"),  type = "b", # ylab=expression(delta^15~"N [\u2030]")  \211 on mac  paste(delta^{15},N," [","\u2030","  ]")
    
     xlab=expression(italic(Delta)[N2O] ~ "[ppb]"), col = "grey" )
...
polygon(c(Dppb,rev(Dppb)),c(sim.mean$bulk_source + sim.sd$bulk_source,rev(sim.mean$bulk_source - sim.sd$bulk_source)),
        col=rgb(0,0.2,0,0.3),border=NA)

polygon(c(Dppb,rev(Dppb)),c(sim.mean$SP_source + sim.sd$SP_source,rev(sim.mean$SP_source - sim.sd$SP_source)),
          col=rgb(0.05,0.05,0.05,0.3),border=NA)

points(Dppb, sim.mean$d18O_source, ylim = c(0,50), col = kulerorange, type = "b")


polygon(c(Dppb,rev(Dppb)),c(sim.mean$d18O_source + sim.sd$d18O_source,rev(sim.mean$d18O_source - sim.sd$d18O_source)),
        col=rgb(0.1,0.1,0.1,0.3),border=NA)

points(Dppb, sim.mean$bulk_source, ylim = c(0,50), col = "black", type = "b")



legend("bottomright",c("bulk 15N","SP","18O"),
       col=c("black","grey",kulerorange),pch = 21)  # ,bty="n" ,pch=16
